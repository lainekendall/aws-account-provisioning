...
service_now_table_name = 'intern-service-now' # REAL: 'aa-servicenowinput'
service_now_primary_key = 'EmailAddress'

credentials_table_name = 'intern-credentials' # REAL: 'aa-accountdetails'
credentials_primary_key = 'AccountNumber'

parameters_table_name = 'intern-parameters' # REAL: 'aa-cfnparameters'
parameters_primary_key = 'AccountNumber'

## SQS ##
sqs_cloudtrail_queue_name = 'intern-cloudtrail' # REAL: 'cloudtrail-notifications'
sqs_config_queue_name = 'intern-config' # REAL: 'config-notifications'

## S3 Buckets/urls ##
s3_config_bucket_name = 'intern-test-config' # REAL: 'adsk-eis-awsconfig'
s3_cloudtrail_bucket_name = 'intern-test-cloudtrail' # REAL: 'adsk-eis-cloudtrail'
s3_account_automation_bucket_name = 'nltest1' # REAL: 'adsk-eis-accountautomation'

cfn_template_url_2azs = 'https://s3-us-west-1.amazonaws.com/nltest1/core-2az.json'
cfn_template_url_3azs = 'https://s3-us-west-1.amazonaws.com/nltest1/core-3az.json'


## SNS Topics ##
sns_config_topic_name = 'intern-test-config-notifications' # REAL: 'config-notifications'

## Lambda ##
lambda_create_linked_func_name = 'intern-test-create-linked'
lambda_cfn_func_name = 'intern-test-main'
lambda_vpc_func_name = 'intern-test-vpc'
lambda_config_func_name = 'intern-laine-config'
lambda_sqs_func_name = 'intern-sqs'

## IAM ##
saml_provider_name = 'adsk-saml'        # REAL: 'adsk-saml'
...
cfn_stack_name = 'core-test' # + client_token_generator() # REAL: core
sns_cfn_topic_name = 'intern-test-stack'

sns_cloudtrail_topic_name = 'laine-test-cloudtrail-notifications' # NEW: 'cloudtrail-notifications'
...
new_saml_provider_name = 'intern-test-adsk-saml' # REAL: 'adsk-saml'
new_saml_role_name = 'intern-test-saml-role'
new_saml_role_policy_name = 'intern-test-saml-role-policy'

s3_automation_file_name = 'accountautomation-test' # 
...
def get_service_now_item():
	...
	# print(type(new_default_regions))
	# new_default_regions = str(new_default_regions)
	# print(type(new_default_regions))
	# new_default_regions = eval(new_default_regions)
	# print(type(new_default_regions))
...
get_queue_url_config = client_sqs.get_queue_url(
    QueueName=sqs_config_queue_name,
    QueueOwnerAWSAccountId=central_account_number)
sqs_cloudtrail_queue_url = get_queue_url_ct['QueueUrl']
sqs_config_queue_url = get_queue_url_config['QueueUrl']
...
#             'Charset': 'string'
        ...
        'Body': { #create AD security group (environment, short projectname, accountnumber) and create email address
            'Text': {
...
initial_queue_policy = {
  "Version": "2008-10-17",
  "Id": "arn:aws:sqs:us-east-1:************:intern-cloudtrail/SQSDefaultPolicy",
  "Statement": [
    {
      ...
      "Resource": "arn:aws:sqs:us-east-1:************:intern-cloudtrail",
      "Condition": {
        "ArnEquals": {
          "aws:SourceArn": [
            "arn:aws:sns:*:************:cloudtrail-notifications",
            "arn:aws:sns:*:************:cloudtrail-notifications"
          ]
        }
      }
    },
    {
      ...
      "Condition": {
        "ArnEquals": {
          "aws:SourceArn": "arn:aws:sns:sa-east-1:************:cloudtrail-notifications"
        }
      }
    }
  ]
  ...
  "Statement": [
    {
      ...
      # "Resource": "arn:aws:sns:us-west-1:************:intern-test-stack", this is the topic arn in new account, in each region stacks are
      "Condition": {
        "StringEquals": {
          # "AWS:SourceOwner": "************" new account number
        }
      }
    },
    {
      ...
      "Effect": "Allow",
      "Principal": {
        # "AWS": "arn:aws:iam::************:root" central account root arn
      },
      "Action": [
        "SNS:Subscribe",
        "SNS:Receive"
      ],
      # "Resource": "arn:aws:sns:us-west-1:************:intern-test-stack", # topic arn
      "Condition": {
        "StringEquals": {
          "SNS:Protocol": "lambda"
        }
...
def delete_bucket_policy(service):
	client_s3.put_bucket_policy(
	    Bucket=s3_cloudtrail_bucket_name,								
...
def delete_sns_config_policy():
	for region in region_names_list:
		client_sns_central = session_new.client('sns', aws_access_key_id=central_account_key, aws_secret_access_key=central_account_secret_key, region_name=region)                        
		sns_config_topic_arn = "arn:aws:sns:" + region + ":" + central_account_number + ":" + sns_config_topic_name
		sns_config_initial_policy_json = json.dumps(sns_config_initial_policy)
		client_sns_central.set_topic_attributes(
		TopicArn=sns_config_topic_arn,
		AttributeName='Policy',
		AttributeValue=sns_config_initial_policy_json
		)
...
def delete_saml_provider():
	...
	for saml_provider in saml_providers:
		...
		if saml_name == saml_provider_name:
